...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--annotators',
                        type=str,
                        default='Nisha shur iosakwe AndreaMariaC',
                        help='Annotators')
    ...
    parser.add_argument('-m', '--mode',
                        type=str,
                        default='double',
                        help='Specify the context format: "single" for one context per line, or "double" for two contexts per line')
    ...
    for k,v in {'train':train, 'dev':dev, 'test':test}.items():
        ...
        v.to_json(f'datasets/{k}-{args.subtask}.jsonl', orient='records', lines=True)
