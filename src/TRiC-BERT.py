...
header = ['model'] + [f'{data_set}-{column}' for data_set in ['train', 'test.in', 'test.out', 'dev'] for column in ['spearman_corr', 'spearman_pvalue', 'pearson_corr', 'pearson_pvalue', 'f1_score']] + ['thr']
header = "\t".join(header)
...
lines.append(f'{model_name}_L{layer}\t' + "\t".join([f'{mask_spearman_corr[i]}\t{mask_spearman_pvalue[i]}\t{mask_pearson_corr[i]}\t{mask_pearson_pvalue[i]}\t{mask_f1_scores[i]}' for i in range(4)]) + f'\t{mask_thr}\n')
